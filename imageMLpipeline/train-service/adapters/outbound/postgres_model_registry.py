"""Postgres adapter: read and register model versions in ``inference_model``.

Reads the model registry seeded/maintained by the Flyway migrations in
``migrations/`` (see ``V1__init.sql``) to discover which model version to retrain,
and inserts a new ``patch + 1`` candidate row once a retrain produces a new memory
bank in object storage.
"""
...
class PostgresModelRegistry(ModelRegistry):
    ...
    def next_version(self, base: ModelVersion) -> ModelVersion:
        with self._connection() as conn, conn.cursor() as cur:
            cur.execute(_MAX_PATCH_SQL, (base.model_type, base.major, base.minor))
            (max_patch,) = cur.fetchone()
